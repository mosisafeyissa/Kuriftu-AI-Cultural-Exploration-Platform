...
class ApiService {
  ...
  static Future<VillaGuide> getVillaGuide(String qrCode) async {
    if (useMockData) {
      await Future.delayed(const Duration(milliseconds: 600));
      // For mock, just return the first villa as a guide if it exists
      if (_mockVillas.isNotEmpty) {
        final villa = _mockVillas.first;
        return VillaGuide(
          id: villa.id,
          name: villa.name,
          imageUrl: villa.image,
          welcomeStory: 'Mock Welcome Story for ${villa.name}',
        );
      }
      throw Exception('Mock data not available');
    }

    try {
      debugPrint('[ApiService] GET $_baseUrl/villa-guide/$qrCode/');
      final response = await http.get(Uri.parse('$_baseUrl/villa-guide/$qrCode/'));
      
      if (response.statusCode == 200) {
        return VillaGuide.fromJson(jsonDecode(response.body));
      }
      debugPrint('[ApiService] Guide failed: ${response.statusCode}');
      throw Exception('Villa guide not found. Please scan a valid QR code.');
    } catch (e) {
      debugPrint('[ApiService] Guide error: $e');
      rethrow;
    }
  }

  static Future<VillaGuide> translateGuide(String qrCode, String lang) async {
    if (useMockData) {
      await Future.delayed(const Duration(milliseconds: 1000));
      return getVillaGuide(qrCode);
    }

    try {
      debugPrint('[ApiService] POST $_baseUrl/villa-guide/$qrCode/translate/ ($lang)');
      final response = await http.post(
        Uri.parse('$_baseUrl/villa-guide/$qrCode/translate/'),
      ...
      debugPrint('[ApiService] Translation error: $e');
      rethrow;
    }
  }
