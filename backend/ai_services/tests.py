...
class AIScanEndpointTest(TestCase):
    ...
    def test_text_fallback_not_found(self, mock_identify):
        """Test returning 404 when artifact is not in DB."""
        response = self.client.post(
            "/api/ai/scan/",
            {"artifact_name": "Non Existent Item"},
            format="json",
        )
    ...
    @patch("ai_services.services.identify_object")
    def test_database_match_success(self, mock_identify):
        """Test successful matching against DB."""
        from artifacts.models import Artifact, Country
        country = Country.objects.create(name="Ethiopia", code="ET")
        ...
        # Mock identify to return a name that should match via icontains/keyword
        mock_identify.clear()
        response = self.client.post(
            "/api/ai/scan/",
            {"artifact_name": "Cultural Chair"},
            format="json",
        )
        
        self.assertEqual(response.status_code, http_status.HTTP_200_OK)
        self.assertEqual(response.data["artifact_name"], "Ethiopian Cultural Chair")
